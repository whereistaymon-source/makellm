...
protected:
    ...
    void onGenerate(CCObject*) {
        ...
        log::info("MakeLLM: config url={} model={} temp={}",
                  m_client.baseUrl, m_client.model, m_client.temperature);
    ...
    void onResponse(web::WebResponse resp) {
        log::info("MakeLLM: response HTTP {}", resp.code());
    ...
    void fail(const std::string& msg) {
        ...
        status("Failed — see the alert for details", {255, 120, 120});
        log::error("MakeLLM: {}", msg);
        FLAlertLayer::create(nullptr, "MakeLLM Error", msg, "OK", nullptr, 360.f)->show();
    }
